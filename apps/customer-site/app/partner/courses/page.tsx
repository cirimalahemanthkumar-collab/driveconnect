...
export default function PartnerCoursesPage() {
  ...
  async function toggleCourseStatus(course: ApiRecord) {
    ...
    try {
      ...
      if (editingCourse && getCourseId(editingCourse) === courseId) {
        resetForm();
      }
  ...
  return (
    <div>
      <PageIntro
        ...
        title="Courses"
        description="Publish packages that customers can discover and compare."
        action={
      ...
      <Card className="mt-6">
        <form className="grid gap-4" onSubmit={save}>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Field label="Course title">
              <Input
                ...
                onChange={(event) =>
                  updateForm(setForm, "course_name", event.target.value)
                }
                required
              />
            </Field>
            ...
            <Field label="Vehicle type">
              <select
                className="focus-ring w-full rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm shadow-sm"
                value={form.vehicle_type}
                onChange={(event) =>
                  updateForm(setForm, "vehicle_type", event.target.value)
                }
                required
              >
            ...
            <Field label="Course type">
              <select
                ...
                onChange={(event) =>
                  updateForm(setForm, "course_type", event.target.value)
                }
              >
                <option value="">Select</option>
            ...
            <Field label="Transmission">
              <select
                ...
                onChange={(event) =>
                  updateForm(setForm, "transmission", event.target.value)
                }
                required
              >
                <option value="">Select</option>
            ...
            <Field label="Total sessions">
              <Input
                ...
                onChange={(event) =>
                  updateForm(setForm, "total_sessions", event.target.value)
                }
                type="number"
                required
              />
            ...
            <Field label="Duration days">
              <Input
                ...
                onChange={(event) =>
                  updateForm(setForm, "duration_days", event.target.value)
                }
                type="number"
                required
              />
            ...
            <Field label="Advance amount">
              <Input
                ...
                onChange={(event) =>
                  updateForm(setForm, "advance_amount", event.target.value)
                }
                type="number"
              />
            </Field>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            {isEditing ? (
              <Button type="button" variant="ghost" className="w-fit" onClick={resetForm}>
                Cancel
              </Button>
            ) : null}
          </div>
      ...
      <TableCard
        columns={["Course", "Vehicle", "Transmission", "Sessions", "Price", "Status", "Actions"]}
          ...
          return (
            <tr key={courseId || `${text(course, "course_name", "title", "name")}-${index}`}>
              <td className="whitespace-nowrap px-4 py-4 font-semibold text-slate-900">
                {text(course, "course_name", "title", "name")}
              </td>

              <td className="whitespace-nowrap px-4 py-4 text-slate-700">
                {vehicleTypeLabel(text(course, "vehicle_type", "vehicleType"))}
              </td>
              ...
              <td className="whitespace-nowrap px-4 py-4 text-slate-700">
                {text(course, "total_sessions", "totalSessions", "sessions")}
              </td>

              <td className="whitespace-nowrap px-4 py-4 text-slate-700">
                <span className="font-bold">{money(text(course, "price"))}</span>
              </td>
...
function updateForm(
  setForm: (value: (current: CourseForm) => CourseForm) => void,
